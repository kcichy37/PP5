...
def delete_favourites(request, favourite_id):
    ...
    if request.user == favourite.user:
        favourite.delete()
        messages.success(request, f'{favourite.product.name} has been removed from your favourites.')
    else:
        messages.error(request, 'You do not have permission to remove this product from your favourites.')
